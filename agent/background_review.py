...
_SKILL_REVIEW_PROMPT = (
    ...
    "Target shape of the library: CLASS-LEVEL skills, each with an "
    "index-style SKILL.md and a `references/` directory carrying the "
    "detail. Not a long flat list of narrow one-session-one-skill "
    "entries. This shapes HOW you update, not WHETHER you update.\n\n"
    "SKILL.md SIZE DISCIPLINE (hard rule): a SKILL.md is a ROUTING "
    "INDEX, not a knowledge store — keep every SKILL.md under ~25,000 "
    "characters. New troubleshooting knowledge, case detail, or any "
    "guidance longer than one sentence goes into `references/<topic>.md` "
    "as a complete standalone doc; the SKILL.md gains exactly ONE index "
    "line (trigger keywords / exact error strings -> the reference "
    "path). NEVER append multi-sentence guidance paragraphs to a "
    "SKILL.md body — if the detail matters, it belongs in the reference "
    "file. If the SKILL.md you want to patch is already over budget, add "
    "only the one index line and do not grow the body further.\n\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "Target shape of the skill library: CLASS-LEVEL skills with an "
    "index-style SKILL.md and a `references/` directory carrying the "
    "detail. Not a long flat list of narrow one-session-one-skill "
    "entries.\n\n"
    "SKILL.md SIZE DISCIPLINE (hard rule): a SKILL.md is a ROUTING "
    "INDEX, not a knowledge store — keep every SKILL.md under ~25,000 "
    "characters. Detail longer than one sentence goes into "
    "`references/<topic>.md` as a complete standalone doc; SKILL.md "
    "gains exactly ONE index line (trigger keywords / exact error "
    "strings -> the reference path). NEVER append multi-sentence "
    "guidance paragraphs to a SKILL.md body; if it is already over "
