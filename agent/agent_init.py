...
def _resolve_bot_mode_protocol_scope(raw) -> str:
    """Normalise ``agent.bot_mode_protocol_scope`` from config (idcsre patch).

    "bot_chat" (default, upstream behaviour) restricts the Bot Mode teammate protocol to a
    profile's canonical "Bot Chat" session; "all" widens it to every ordinary session of a
    Bot-Mode-managed profile (group rooms, cron agents and subagents stay excluded — see
    ``tools/bot_mode_dm.bot_mode_scope_allows``). Anything else is a config typo: warn and fall
    back to the safe default rather than silently widening or narrowing the A2A surface.
    """
...
def _apply_agent_section(agent, _agent_cfg):
    ...
    # idcsre patch: how far the Bot Mode teammate protocol reaches — "bot_chat" (upstream: the
    # canonical "Bot Chat" session only) or "all" (every session of a Bot-Mode-managed profile).
    # tools/bot_mode_dm.bot_mode_scope_allows is the single shared gate; group rooms, cron agents
    # and subagents stay excluded either way.
