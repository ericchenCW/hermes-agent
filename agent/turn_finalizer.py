...
def finalize_turn(
    ...
    # idcsre patch — secret-redaction backstop for the FINAL user-visible reply.
    # Tool results are already redacted at each tool boundary (read_file / search_files /
    # terminal), but the model can quote a secret it saw in a tool result or system prompt straight
    # into its answer. This is the one chokepoint every delivery surface (gateway, CLI, TUI,
    # oneshot) passes through, so the mask is applied once here. force=True is NOT used:
    # operators who deliberately set security.redact_secrets: false keep their opt-out, same as
    # every other redaction site.
