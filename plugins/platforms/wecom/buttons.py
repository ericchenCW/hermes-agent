...
class WeComButtonsMixin:
    ...
    async def _on_template_card_event(self, payload: Dict[str, Any]) -> None:
        ...
        if self._dedup.is_duplicate(msg_id):
            return
        # 2) Reply on the event's req_id: WeCom accepts several passive responses per req_id
        #    (finish frame + template_card verified), so the click turn streams like a normal turn.
        #    _button_click_chats keeps the proactive fallback open in groups if that ever fails.
        if req_id:
