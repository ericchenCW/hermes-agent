...
def read_file_tool(path: str, offset: int = 1, limit: int = DEFAULT_READ_LIMIT, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Device path guard ─────────────────────────────────────────
        # Block paths that would hang the process (infinite output, blocking on input). Pure path
        # check — no I/O. Operates on the already-resolved absolute path, so no base_dir is needed
        # and symlink-chasing starts from the same place the read guard judged.
