...
def get_model_info(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # The resolution chain probes /models on the endpoint; it must carry the same bearer
            # as the chat request or a keyed endpoint 401s and we report the default window as
            # "auto-detected".
            try:
            ...
            auto_ctx = get_model_context_length(model=model_name, base_url=base_url, api_key=_probe_key,
