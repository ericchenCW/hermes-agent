...
Covers:
  ...
  hermes_cli/web_routers/models.py — GET /api/model/info context-length probe
...
def test_model_info_context_probe_carries_api_key(monkeypatch):
    models_router = pytest.importorskip("hermes_cli.web_routers.models")

    # Upstream moved /api/model/info into hermes_cli.web_routers.models (web_server
    # re-exports it lazily); the handler resolves load_config in that module.
    monkeypatch.setattr(models_router, "load_config", _model_config)
    monkeypatch.setattr(rp, "load_config", _model_config)

    ctx_probe = MagicMock(return_value=262144)
    with patch("agent.model_metadata.get_model_context_length", ctx_probe):
        models_router.get_model_info()
