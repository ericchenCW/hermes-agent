...
class TestCrossSessionPrefixStability:
    ...
    def _prompt_for(self, session_id, cwd):
        ...
        with (
            patch("agent.prompt_builder.load_soul_md", return_value="You are an agent. Persona.\n" * 20),
            patch("agent.prompt_builder.build_skills_system_prompt",
    ...
    def test_only_the_env_block_moved(self, monkeypatch, tmp_path):
        ...
        # Upstream wraps the block in its runtime-environment markers, so removing the hints drops
        # three paragraphs, not one.
        env_blocks = [
            p for p in with_env
            if p.startswith("Host: ") or p.startswith("# Hermes runtime environment")
            or p.startswith("<!-- End Hermes runtime environment")
        ]
        assert len(env_blocks) == 3, env_blocks
        assert any("Current working directory:" in b for b in env_blocks)
