...
def deployment(monkeypatch, tmp_path: Path):
    ...
    (kb / "hoshino-it-support").mkdir(parents=True)
    (kb / "hoshino-it-support" / "a.md").write_text("kb line\n")
    out = tmp_path / "out"
    out.mkdir()
    (out / "report").mkdir()
...
class TestReadguardAudit:
    ...
    def test_quota_denial_logged(self, deployment, monkeypatch):
        ...
        docs = deployment["kb"] / "hoshino-it-support"
...
class TestExportForbidden:
    def _commands(self, kb: Path, out: Path) -> list[str]:
        return [
            ...
            f"cp -r {kb}/hoshino-it-support {out}/",
            f"rsync -a {kb}/ {out}/",
            f"find {kb} -name '*.md' -exec cp {{}} {out} \\;",
            f"zip -r {out}/kb.zip {kb}/hoshino-it-support",
            f'python3 -c "import shutil;shutil.copytree(\'{kb}/hoshino-it-support\',\'{out}/x\')"',
        ]
    ...
    @pytest.mark.parametrize(
        ...
        [
            "cp -R {kb} {out}/",
            "cp -a {kb}/hoshino-it-support {out}/",
            "tar cf - {kb}",
            "7z a {out}/kb.7z {kb}",
            "find {kb} -type f | xargs cp -t {out}",
            "cat {kb}/hoshino-it-support/* > {out}/all.md",
    ...
    @pytest.mark.parametrize(
        "shape",
        [
            "cp {kb}/hoshino-it-support/a.md {out}/",
            "tar czf {out}/a.tgz {out}/report",
            "cat {kb}/hoshino-it-support/a.md",
            "grep -n line {kb}/hoshino-it-support/a.md",
        ],
    )
    ...
    def test_terminal_returns_the_export_payload(self, deployment):
        ...
        raw = terminal_tool.terminal_tool(
            command=f"cp -r {deployment['kb']}/hoshino-it-support {deployment['out']}/"
        )
        payload = json.loads(raw)
    ...
    def test_terminal_allows_single_file_copy(self, deployment):
        ...
        raw = terminal_tool.terminal_tool(
            command=(
                f"cp {deployment['kb']}/hoshino-it-support/a.md {deployment['out']}/"
            )
        )
        payload = json.loads(raw)
...
class TestKbReadQuota:
    def _kb_docs(self, kb: Path, count: int) -> list[Path]:
        ...
        for i in range(count):
            doc = kb / "hoshino-it-support" / f"doc-{i}.md"
    ...
    def test_bypass_lifts_the_quota(self, deployment, monkeypatch):
        ...
        doc = deployment["kb"] / "hoshino-it-support" / "a.md"
        for _ in range(5):
