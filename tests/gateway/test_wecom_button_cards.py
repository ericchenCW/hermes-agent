...
# the feature flag lives in the buttons mixin module; patch it there
...
def test_buttons_disabled_strips_directive_and_sends_no_card():
    ...
    ad = _adapter()
    sent = _wire(ad)
    assert mb.BUTTONS_ENABLED is True  # enabled by the env set at import time
    mb.BUTTONS_ENABLED = False
    try:
