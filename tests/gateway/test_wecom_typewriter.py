...
# the pacer constants live in the streaming mixin module; patch them there
ms = importlib.import_module("plugins.platforms.wecom.streaming")
...
def test_pacer_reveals_gradually_then_finalize(monkeypatch):
    monkeypatch.setattr(ms, "TYPEWRITER_TICK_SECONDS", 0.02)
    async def main():
        ad, sent = _adapter()
...
def test_pacer_respects_frame_budget(monkeypatch):
    monkeypatch.setattr(ms, "TYPEWRITER_TICK_SECONDS", 0.01)
    async def main():
        ad, sent = _adapter()
        await ad.send_stream_frame("", chat_id="c1", turn_id="t1")
        turn = ad._stream_turns["c1:t1"]
